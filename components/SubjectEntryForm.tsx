...
// ✅ INTERNAL TYPES (Self-contained, No external imports needed)
...
const SubjectEntryForm: React.FC<SubjectEntryFormProps> = ({ 
  ...
  // --- 3. MARKS LOGIC (65 Marks Rule Included) ---
  ...
  const canEdit = useMemo(() => {
    ...
    // RULE A: Admin is ALWAYS allowed
    if (role === 'ADMIN') return true;

    // RULE B: Incharge is allowed in OWN CLASS
    if (role === 'CLASS_INCHARGE' && currentUser.assignedClass === selectedClass) {
      return true;
    }

    // RULE C: Subject Teacher allowed for OWN SUBJECT
  ...
  const handleCommit = () => {
    ...
    for (const s of currentClassStudents) {
      ...
      if (mark > maxMarks) {
        alert(`Error: Roll No ${s.rollNo} marks cannot exceed ${maxMarks}`);
        return;
      }
    }
  ...
  const downloadAwardList = () => {
    const currentClassStudents = allData.filter((s:any) => s.class === selectedClass || s.className === selectedClass);
    if (currentClassStudents.length === 0) { alert("No students found."); return; }

    const headers = ['Roll No', 'Student Name', `Marks (Max: ${maxMarks})`];
    const rows = currentClassStudents.map((s:any) => [
      s.rollNo, `"${s.name}"`, localMarks[s.rollNo] || '0'
    ]);
    ...
    <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
      ...
      <div className="p-6 bg-slate-900 text-white">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex flex-wrap gap-2 text-black">
            <select value={selectedClass} onChange={e => setSelectedClass(e.target.value)} className="px-3 py-2 rounded-lg font-bold text-sm bg-indigo-50 border-2 border-indigo-200 cursor-pointer">
              {['6th','7th','8th','9th','10th'].map(c => <option key={c} value={c}>Class {c}</option>)}
            </select>

            <select value={examType} onChange={e => setExamType(e.target.value)} className="px-3 py-2 rounded-lg font-bold text-sm bg-indigo-50 border-2 border-indigo-200 cursor-pointer">
            ...
            <select value={selectedSubKey} onChange={e => setSelectedSubKey(e.target.value)} className="px-3 py-2 rounded-lg font-bold text-sm bg-indigo-50 border-2 border-indigo-200 min-w-[120px] cursor-pointer">
      ...
      {!canEdit ? (
        <div className="bg-gray-200 p-2 text-center text-xs font-bold text-gray-500 uppercase tracking-widest border-b border-gray-300">
          🔒 Read Only Mode: No Permission for this subject/class.
        </div>
      ) : (
